    ...
    for (int row = 0; row < m; row++) {
        for (int col = mat_args->start_j; col < mat_args->end_j; col++) {
            ...
            for (int q = 0; q < num_block; q += 4) {
                ...
                __m256i low_half = _mm256_and_si256(raw_w, lowMask);
                __m256i high_half = _mm256_and_si256(_mm256_srli_epi16(raw_w, 4), lowMask);
                
                __m256i low_half_next = _mm256_and_si256(raw_w_next, lowMask);
                __m256i high_half_next = _mm256_and_si256(_mm256_srli_epi16(raw_w_next, 4), lowMask);
                // TODO: apply zero_point to weights and convert the range from (0, 15) to (-8, 7)
                ...
                w_0 = _mm256_sub_epi8(low_half, zero_point);
                w_128 = _mm256_sub_epi8(high_half, zero_point);
                w_0_next = _mm256_sub_epi8(low_half_next, zero_point);
                w_128_next = _mm256_sub_epi8(high_half_next, zero_point);
...
void MatmulOperator::mat_mul_all_techniques(struct matmul_params *params) {
    ...
    int n = C->column;
    int cols_per_thread = (n + num_thread - 1) / num_thread;

    for (int i = 0; i < num_thread; i++) {
        threads_args[i] = {i*cols_per_thread, std::min((i + 1) * cols_per_thread, n), params};
